...
class BookingAdmin(admin.ModelAdmin):
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        ...
        extra_context['previous_month'] = reverse('admin:bookings_booking_change', args=(object_id,)) + '?start__gte=' + str(previous_month)
        extra_context['next_month'] = reverse('admin:bookings_booking_change', args=(object_id,)) + '?start__gte=' + str(next_month)
 
        cal = BookingSlotCalendar()
        html_calendar = cal.formatmonth(object_id, d.year, d.month, withyear=True)
