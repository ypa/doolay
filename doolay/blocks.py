...
from wagtail.core.blocks import (StructBlock, TextBlock, StreamBlock,
                                 RichTextBlock, CharBlock, ChoiceBlock)
...
class GlobalStreamBlock(StreamBlock):
    ...
    header = StructBlock(
        [('header_text', CharBlock(blank=True, required=False,
                                   label='Header')),
         ('size',
          ChoiceBlock(
              choices=[('', 'Select a header size'), ('h2', 'H2'),
                       ('h3', 'H3'), ('h4', 'H4')],
              blank=True,
              required=False))],
        classname="title",
    ...
    image = StructBlock(
        [('image', ImageChooserBlock()),
         ...
         ('style',
          ChoiceBlock(
              choices=[('', 'Select an image size'), ('full', 'Full-width'),
                       ('contain', 'Contained-width'), ('half', 'Half-width')],
              required=False))],
        icon="image",
    ...
    blockquote = StructBlock(
        [
            ('text', TextBlock()),
            ('attribute_name',
             CharBlock(blank=True, required=False,
